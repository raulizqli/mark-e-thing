...
export default function GeneratePage() {
  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    try {
      const response = await api.post<{ results: GenerateContentResult[]; contents: Content[] }>(
        `/companies/${params.id}/generate`,
        { types: selectedTypes, topic: topic || undefined },
      );
      setResults(response.results ?? []);
      setSavedContents(response.contents ?? []);
    } catch (err) {
  ...
  async function handleGenerateImage(index: number, prompt: string, contentId?: string) {
    ...
    try {
      const image = await api.post<GeneratedImage>(
        `/companies/${params.id}/generate/image`,
        { prompt, contentId },
      );
